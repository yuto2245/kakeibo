...
```tsx
client/
├── public/                 # 静的ファイル（create-react-appデフォルト）
│   └── index.html          # エントリHTML
├── src/                    # メインソース
│   ├── components/         # UI部品
│   │   ├── AuthButton.tsx  # Zaim OAuthボタン（バックエンド/auth呼び出し）
│   │   └── Dashboard.tsx   # データ表示コンポーネント
│   ├── pages/              # ページ単位
│   │   ├── Login.tsx       # ログイン画面
│   │   └── Home.tsx        # ホーム画面
│   ├── services/           # APIラッパー
│   │   └── api.ts          # Axiosインスタンス（バックエンドURL設定）
│   ├── utils/              # ユーティリティ
│   │   └── authUtils.ts    # JWT管理
│   ├── App.tsx             # ルートコンポーネント（Router設定）
│   ├── index.tsx           # ReactDOMエントリ
│   └── types.ts            # 型定義（インターフェース集）
├── .env                    # 環境変数（REACT_APP_API_URL=バックエンドURL）
├── tsconfig.json           # TypeScript設定
└── package.json            # client専用依存
```
...
├── src/                    # ソースコードのメイン
│   ├── config/             # 設定関連
│   │   └── db.ts           # データベース接続ロジック（MongoDB URIを環境変数から読み込み）
│   ├── controllers/        # リクエストハンドラ
│   │   ├── authController.ts  # OAuthフロー（/auth, /callback）の処理。oauth-1.0aで署名生成を実装
│   │   └── userController.ts  # ユーザーCRUD（登録、更新、削除、取得）。JWT検証をミドルウェアで適用
│   ├── middlewares/        # 共通処理
│   │   └── authMiddleware.ts  # JWT認証チェック。jsonwebtoken.verifyを使用
│   ├── models/             # データモデル
│   │   └── User.ts         # Mongooseスキーマ（email, passwordハッシュ, zaimトークン）。bcryptでハッシュ化
│   ├── routes/             # エンドポイント定義
│   │   ├── authRoutes.ts   # OAuth関連ルート（/api/zaim/auth, /api/zaim/callback）
│   │   └── userRoutes.ts   # ユーザー関連ルート（/api/users/register など）
│   ├── services/           # ビジネスロジック
│   │   └── zaimService.ts  # Zaim API呼び出しプロキシ（/v2/home/money の署名付きリクエスト）
│   └── index.ts            # サーバー起動エントリ（Expressアプリの設定、ルートマウント、ポートリスニング）
├── .env                    # 環境変数（ZAIM_CONSUMER_KEY, DB_URI など）。Vercelで同期
├── tsconfig.json           # TypeScriptコンパイラ設定（strict: true で型安全確保）
└── package.json            # 依存リストとスクリプト（例: "start": "ts-node src/index.ts"）
```
